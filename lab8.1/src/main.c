...
*/
int num_in_words (int input);
/**
функція для знаходження НСД
*/
int NSD_function (int number_one, int number_two);
...
srand(time(NULL));
	...
	int num_in_words (int input)	/**Перша функція*/
{
char output[6][15] = {};	/**масив з виводом значень*/
...
int numArray[5] = {'\0'}; //number array for first function
	...
	int num_pos = 4;	/**Змінна для позиції числа в масиві*/
			...
			for (int j = 0; j < 4; j++) {	/**цикл, щоб передати номер слова в масиві зі словами*/
				for (int i = 0; i < 15; i++) {	/**цикл, щоб вивести побуквенно позначення цифри*/
					if (numArray[3] == 1 && j < 1) {	/**якщо введене значення має подвійне число 11,12,13*/
						num_pos = 5;
		...
		for (int i = 0; i < 2; i++) {	/**виведення буквенного значення на екран*/
			for (int j = 0; j < 1; j++) {
	printf("%s", &output[i][j]);
}
}
	...
	int NSD_function (int number_one, int number_two)	/**Друга функція*/
{
